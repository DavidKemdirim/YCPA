...
% for every region set up the parameters
...
for k = 1:Reg.n
    ...
    figure
    subplot(1,2,1),surface((xC{k}+Reg.xoff{k}/2)/1e-6,(yC{k}+Reg.yoff{k}/2)/1e-6,epi{k}.','linestyle','none');title('\epsilon');
    subplot(1,2,2),surface((xC{k}+Reg.xoff{k}/2)/1e-6,(yC{k}+Reg.yoff{k}/2)/1e-6,mu{k}.','linestyle','none');title('\mu');
...
for i = 1:nSteps
    ...
    for k = 1:Reg.n
        ...
        % Source and inclusion setup
        if bc{k}.NumS > 0
            
            Einc = bc{k}.s(1).fct(yC{k},i,t,bc{k}.s(1).paras)*20;
            Eincp = bc{k}.s(1).fct(yC{k},i,t-ds/c_c/2,bc{k}.s(1).paras)*20;
